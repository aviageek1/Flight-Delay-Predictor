...
def quantize(df, delay): # parameter delay is int that states how many minutes counts as "delayed" (ex: 15) 
    indices_to_drop = []
    ...
    df = df.drop(indices_to_drop) # actually remove rows that had no delay 

    return df

# count and print how many occurances of the origin/dest pair there are and how many are delayed
"""
def check_occurances(origin, dest): 
    ######## CHECK OCCURANCES
    # originn = 'SLC'
    # destt = 'DTW'
...
# driver
def main():
    print("Welcome to Flight Model Predictor!")
    print("Created by: Aayush Sharma")
    print("\n")
    print("Input a major airport's flight route, time, and minutes-delay you want to calculate by and based on 2008 data,"
          "get a prediction on if it will be delayed or not!")
    print("Please note that commercial air traffic in 2008 took a big hit in terms of on-time performance, " 
    ...
    date = input("Enter the flight's departure date in the format mm/dd : ")
    time = input("Enter the time of the flight in the format hh:mm:ss : ")
    o_airports = input("Enter an origin airport you would like to search for : ")
    d_airports = input("Enter a destination airport you would like to search for : ")
    
    df = origins(o_airports, df)
    
    delay = input("Enter the amount of delay (in minutes) you would like to sort by (how late is too late?): ")
    print("Loading.. please wait")
    df = quantize(df, delay)

    df = dummy(df)
    df, model, accuracy = trees(df)

    departure_date_time = f"{date}/2008 {time}, {o_airports}, {d_airports}"
    probability_on_time = predict_delay(departure_date_time, o_airports, d_airports, df, model)

    print(f"The probability that your selected flight will be on-time is {probability_on_time}.")
